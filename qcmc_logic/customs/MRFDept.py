...
def get_departments_by_user(doctype, txt, searchfield, start, page_len, filters):
    ...
    # 🔒 Normal user — ONLY departments tied to their Role Profile(s)
    return frappe.db.sql("""
        SELECT DISTINCT
            d.name,
            d.department_name
        FROM `tabDepartment` d
        INNER JOIN `tabStaffing Plan Role Assignment Details` spad
            ON spad.department = d.name
        INNER JOIN `tabStaffing Plan Role Assignment` spa
            ON spa.name = spad.parent
        WHERE spa.role_profile IN (
            SELECT urp.role_profile
            FROM `tabUser Role Profile` urp
            WHERE urp.parent = %(user)s
        )
          AND d.company = %(company)s
          AND (
              d.name LIKE %(txt)s
...
def get_staffing_plans_by_department(doctype, txt, searchfield, start, page_len, filters):
    ...
    # 🔒 Normal user — only plans tied to their Role Profile(s)
    return frappe.db.sql("""
        SELECT name
        FROM `tabStaffing Plan`
        WHERE department = %(department)s
          AND name LIKE %(txt)s
          AND name IN (
              SELECT spad.staffing_plan
              FROM `tabStaffing Plan Role Assignment Details` spad
              WHERE spad.parent IN (
                  SELECT spa.name
                  FROM `tabStaffing Plan Role Assignment` spa
                  WHERE spa.role_profile IN (
                      SELECT urp.role_profile
                      FROM `tabUser Role Profile` urp
                      WHERE urp.parent = %(user)s
